...
def get_dataloaders(idx, labels_np, batch_size=None):
    ...
    dataloaders = {phase: DataLoader(dataset=dataset, batch_size=batch_size, shuffle=False)
                   for phase, dataset in datasets.items()}
    return dataloaders


def train_model(name: str, model_class: Type[nn.Module], graph: SparseGraph, model_args: dict,
        learning_rate: float, reg_lambda: float,
        idx_split_args: dict = {'ntrain_per_class': 10, 'nstopping': 50, 'nknown': 150, 'seed': 2413340114},
